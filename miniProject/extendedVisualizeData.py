...
def update_plot(attrName,old,new):
    ...
    if curdoc().get_model_by_name('mainPlot')!=None:
        curdoc().clear()
        # mainPlotObject=curdoc().get_model_by_name('mainLayout')
        # curdoc().get_model_by_name('mainLayout').children.remove(mainPlotObject)
        #rangePlotObject = curdoc().get_model_by_name('rangePlot')
        #curdoc().get_model_by_name('mainLayout').children.remove(rangePlotObject)
    p = figure(name="mainPlot",y_range=genres, x_axis_type="datetime",x_range=(x[0], x[len(x)-1]),plot_height=500,plot_width=1000, x_axis_label="date", y_axis_label = "genres")
    ...
    for i, genre in enumerate(reversed(genres)):
        ...
        if type=='Full':
            source = ColumnDataSource(data={'x': x, 'y': y,'energyLowPercent':getTooltipDataForGenre(genre,'energyLowPercent',type),'energyHighPercent':getTooltipDataForGenre(genre,'energyHighPercent',type),'tempoSlowPercent':getTooltipDataForGenre(genre,'tempoSlowPercent',type),'tempoMediumPercent':getTooltipDataForGenre(genre,'tempoMediumPercent',type),'tempoFastPercent':getTooltipDataForGenre(genre,'tempoFastPercent',type)})
        else:
